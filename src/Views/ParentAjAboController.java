...
public class ParentAjAboController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        type.getItems().addAll("bus","normal");
         try{
            ...
            String res="SELECT en.nom,en.prenom,en.id FROM enfant en,parent AS pa WHERE en.parent_id=pa.id AND pa.id="+ids ;
	...
	java.sql.Date date = new java.sql.Date(Calendar.getInstance().getTime().getTime());
       ...
       String text = df.format(date);  
        ...
        en.ajouterAbonnement(1, i, text,type.getSelectionModel().getSelectedItem(), "attente", tx);
